...
- [ ] **Weapon: hitscan first.** Not built as planned — jumped ahead and gave the player the existing `GunComponent`/`Projectile` rig instead of a new raycast `Weapon` node (see Viewmodel, Phase 2, built out of order for the same reason). Hold `fire` (left click) to hold-to-fire at `Interval` (0.2s), same as a turret. Works, and `test_player_gun.tscn` covers it end to end, but it's still a projectile with travel time — the hitscan `Weapon` node this line originally meant is still undone, and "projectiles add lead, travel time, and pooling for no learning" is still true if this needs revisiting.
