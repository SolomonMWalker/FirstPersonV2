...
- [ ] **Hit registration on the enemy.** It already has a `HealthComponent` and a `Dead` state that drops its collider and removes the body, and it can be killed today by walking it through a turret's line of fire. What's missing is a player weapon to do it with, plus a flinch.
