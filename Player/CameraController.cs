...
public partial class CameraController : Camera3D
{
	...
	[Export] public float BobAmount = 0.02f;  // metres, peak vertical rise at full speed
	[Export] public float BobStride = 3f;    // metres travelled per full bob cycle
	[Export] public float BobSway = 0.6f;      // lateral bob as a fraction of the vertical
	[Export] public float RollAngle = 0.75f;    // degrees of roll at full sideways speed
	[Export] public float Smoothing = 10f;      // how fast roll and bob amplitude chase their targets

	// Impact punch — the angular kick from landing. Source's constants, which are per-second against
	// a delta multiply and so port over with no unit conversion. PunchDamping is low enough that the
	// spring overshoots on purpose: the head dips, rebounds a little past level and settles, and that
	// rebound is what makes a landing feel like one.
	...
	// One impulse channel for every impact. Landing calls it with pitch; weapon recoil, when there
	// is a weapon, calls the same thing. Kept as a single system on purpose.
	//
